...
def dashboard(request):
    ...
    # --- Weekly Spending Chart (Improved) ---
    ...
    x_labels = week_days_str
    y_values = [daily_totals.get(day, 0) for day in x_labels]

    plt.figure(figsize=(8, 4))
    plt.plot(x_labels, y_values, color="#3b82f6", marker='o', linewidth=2.5)
    plt.fill_between(x_labels, y_values, color="#3b82f6", alpha=0.2)
    plt.title("Weekly Spending Trend", fontsize=14, weight='bold')
    ...
    plt.savefig(weekly_chart_path, dpi=100)
    plt.close()
